...
public:
   ...
   // bool handshake(Client &client);
    ...
    bool getData(std::string& data, uint8_t *opcode = NULL);

    // Write data to the stream
    void sendData(const char *str, uint8_t opcode = WS_OPCODE_TEXT);
    void sendData(std::string str, uint8_t opcode = WS_OPCODE_TEXT);
...
private:
    ...
    bool handleStream(std::string& data, uint8_t *opcode);
    ...
    void sendEncodedData(std::string str, uint8_t opcode);
};
